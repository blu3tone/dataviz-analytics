...
class edgesProgram(graphProgram):
    ...
    def updateAttributes(self, values):
        ...
        # Both z coord and color are changed to visualize the values
    ...
    def updateZcoords(self, nodeZcoords):
        # nodeZcoords gives the z coord for the nodes 
        # Update the edge endpoint z values to match
        #
        for edgeIdx, edge in enumerate(self.edgeList):
            for ep in edge.edgeId:
                self.nodeData['a_zAdjust'][ep.index]= nodeZcoords[ep.node.index]
    ...
    def updateColors(self, values):
        ...
        # nodes
        # Update the endpoint colors to match:
        
        weights = np.log(values+0.0001)
        minWt=-7
        
        for edgeIdx, edge in enumerate(self.edgeList):
            for nidx in edge.points:
                self.nodeData['a_fg_color'][nidx] = colorSelect(1-weights[edgeIdx]/minWt) 
